...
class Validaciones extends Exception
{
	...
	PUBLIC function validarTelefono($string){// valida telefono con formato "0414-5555555" o "04145555555" o "0414 5555555"
		...
		if(!preg_match($pattern, $string)){
			throw new Validaciones("El teléfono no es valido", 1);
		}
	}
	PUBLIC function validarEmail($string){
		if(!filter_var( $string , FILTER_VALIDATE_EMAIL)){
			throw new Validaciones("El correo no es valido", 1);
		}
	...
	PUBLIC function validarContrasena($string)
	{
		...
		if(!preg_match($pattern, $string)){
			throw new Validaciones("La contraseña no es valida", 1);
		}
	...
	PUBLIC function fecha($string){
		if(is_string($string)){
			if(!preg_match("/^[\d][\d][\d][\d][\D][\d][\d][\D][\d][\d]$/", $string)){
				...
				throw new Validaciones("La fecha no tiene el formato adecuado (yyyy-mm-dd) :: $string", 1);
			}
			$string = preg_split("/\D/", $string);

		}else{throw new Validaciones("la fecha no es una cadena de caracteres", 1);}

		$a = $string[0];// año
		$m = $string[1];// mes
		$d = $string[2];// dia
		if($a < 1900){throw new Validaciones("El año de la fecha no puede ser menor a 1900", 1);}
		if(($m < 1) || $m > 12){throw new Validaciones("El mes de la fecha no puede ser menor a 1 o mayor a 12", 1);}
		if(($d < 1) || $d > 31){throw new Validaciones("El día de la fecha no puede ser menor a 1 o mayor a 31", 1);}
		if(($a%4 != 0) and ($m == 2) and ($d > 28)){throw new Validaciones("Solo el año bisiesto tiene mas de 28 días en febrero", 1);}
		if((($m == 4) || ($m == 6 )|| ($m == 9 )|| ($m == 11) ) and $d>30){throw new Validaciones("El mes seleccionado no puede tener mas de 30 días", 1);}
		if($m==2 and $d>29){throw new Validaciones("El mes de febrero no puede tener mas de 29 días", 1);}
	}

	PUBLIC function hora($string){
		if(!preg_match("/^(?:[01]?[0-9]|2[0-3]):[0-5][0-9]$/", $string)){
			throw new Validaciones("La hora no es valida", 1);
		}
	}

	PUBLIC function monto_Miles($string){
		if(!preg_match("/^[0-9]{1,3}(?:[\.][0-9]{3}){0,6}[,][0-9]{2}$/", $string)){
			throw new Validaciones("El monto es invalido", 1);
